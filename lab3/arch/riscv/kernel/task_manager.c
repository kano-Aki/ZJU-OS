...
void task_init(void) {
  ...
  for(int i = 0; i < LAB_TEST_NUM; ++i) {
    ...
    uint64_t* user_tb = alloc_page();
    // // 3. 将task[i]->sscratch指定为虚拟空间下的栈地址，即0x1001000 + PAGE_SIZE（注意栈是从高地址到低地址使用的）
    task[i]->sscratch = (uint64_t)0x1001000 + PAGE_SIZE;//因为task生成完之后会通过sret退出，退出时又会交换user_sp和kernel_sp，所以刚开始sscratch放置user_sp
    // // 4. 正确设置task[i]->satp，注意设置ASID
    task[i]->satp = ((uint64_t)user_tb >> 12) | (((uint64_t)i) << 44LL) | (0x8000000000000000);
    // // 5. 将用户栈映射到实际的物理地址，使用create_mapping函数
    create_mapping(user_tb, 0x1001000, user_sp, 4 * 1024, PTE_R | PTE_V | PTE_W | PTE_U);
    // // 6. 将用户程序映射到虚拟地址空间，使用create_mapping函数
    create_mapping(user_tb, 0x1000000, task_addr, 4 * 1024, PTE_R | PTE_V | PTE_X | PTE_W | PTE_U);
    // 7. 将将虚拟地址 0xffffffc000000000 开始的 16 MB 空间映射到起始物理地址为 0x80000000 的 16MB 地址空间，注意此时 &rodata_start、... 得到的是虚拟地址还是物理地址？我们需要的是什么地址？
    create_mapping(user_tb, 0xffffffc000000000, 0x80000000, 16 * 1024 * 1024, PTE_R | PTE_W | PTE_X | PTE_V);//此时得到的是虚拟地址
    ...
    create_mapping(user_tb, 0x80000000, 0x80000000, 16 * 1024 * 1024, PTE_R | PTE_W | PTE_X | PTE_V);
    // 9. 修改对内核空间不同 section 所在页属性的设置，完成对不同section的保护，其中text段的权限为 r-x, rodata 段为 r--, 其他段为 rw-，注意上述两个映射都需要做保护。
    
    create_mapping(user_tb, &text_start, PHYSICAL_ADDR(&text_start), (&rodata_start - &text_start) << 3LL, PTE_R | PTE_X | PTE_V);//text只有R和X的权限,指针相减的结果是以uint64*为结果的
    create_mapping(user_tb, &rodata_start, PHYSICAL_ADDR(&rodata_start), (&data_start - &rodata_start) << 3LL, PTE_R | PTE_V);//rodata只有R的权限
    create_mapping(user_tb, &data_start, PHYSICAL_ADDR(&data_start), (&_end - &data_start) << 3LL, PTE_R | PTE_W | PTE_V);//其它有rw-的权限
    
    // 10. 将必要的硬件地址（如 0x10000000 为起始地址的 UART ）进行等值映射 ( 可以映射连续 1MB 大小 )，无偏移，PTE_V | PTE_R 为映射的读写权限
    create_mapping(user_tb, 0x10000000, 0x10000000, 1024 * 1024, PTE_V | PTE_R | PTE_W);//前面的都是7位数，因此都可以使用VIRTUAL_ADDR和PHYSICAL_ADDR两个宏定义
