...
struct mm_struct {
  ...
  uint64_t user_stack;         // 用户栈起始地址(物理)(低位)。使用时应从高位开始使用，即user_stack+PAGE_SIZE为起始栈顶地址
};
...
struct task_struct {
  ...
  struct thread_struct thread; // 该进程状态段 在进程切换时才会用到，保存和恢复

  uint64_t sscratch; // 保存 sscratch 在进程切换时才会用到，保存和恢复
  uint64_t satp;     // 保存 satp 在进程切换时才会用到，保存和恢复
